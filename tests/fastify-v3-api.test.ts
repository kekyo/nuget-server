...
describe('Fastify NuGet V3 API - Phase 3 Tests', () => {
  ...
  describe('V3 API with authMode=none (no authentication)', () => {
    ...
    test('should redirect /v3/ to /v3/index.json', async () => {
      const response = await fetch(`http://localhost:${serverPort}/v3/`, {
        redirect: 'manual',
      });
    ...
    test('should redirect /index.json to /v3/index.json', async () => {
      const response = await fetch(
        `http://localhost:${serverPort}/index.json`,
        {
          redirect: 'manual',
        }
      );
    ...
    test('should redirect / with Accept: application/json to /v3/index.json', async () => {
    ...
    test('should serve UI for / without Accept: application/json', async () => {
      const response = await fetch(`http://localhost:${serverPort}/`, {
        headers: {
          Accept: 'text/html',
        },
