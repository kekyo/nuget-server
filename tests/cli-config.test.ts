...
describe('CLI configuration priority', () => {
  ...
  const runCli = async (args: string = '', env: Record<string, string> = {}): Promise<{ stdout: string; stderr: string }> => {
  ...
  it('should use config.json when CLI and env not provided', async () => {
    ...
    expect(output).toContain(`Loaded configuration from ${testDir}/config.json`);
  }, 10000);
  ...
  it('should handle mixed configuration sources', async () => {
    ...
    const { stdout } = await runCli(`--package-dir ./cli-packages -c ${testDir}`, env);
