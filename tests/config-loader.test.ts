...
describe('config-loader', () => {
  ...
  it('should load valid config.json', async () => {
    ...
    await writeFile(join(testDir, 'config.json'), JSON.stringify(configData, null, 2));
  ...
  it('should validate port range', async () => {
    ...
    for (const testCase of testCases) {
      await writeFile(join(testDir, 'config.json'), JSON.stringify({ port: testCase.port }));
      const config = await loadConfigFromFile(testDir);
  ...
  it('should validate logLevel values', async () => {
    ...
    for (const level of validLevels) {
      await writeFile(join(testDir, 'config.json'), JSON.stringify({ logLevel: level }));
      const config = await loadConfigFromFile(testDir);
      expect(config.logLevel).toBe(level);
    }

    // Invalid level
    await writeFile(join(testDir, 'config.json'), JSON.stringify({ logLevel: 'verbose' }));
    const config = await loadConfigFromFile(testDir);
  ...
  it('should validate authMode values', async () => {
    ...
    for (const mode of validModes) {
      await writeFile(join(testDir, 'config.json'), JSON.stringify({ authMode: mode }));
      const config = await loadConfigFromFile(testDir);
      expect(config.authMode).toBe(mode);
    }

    // Invalid mode
    await writeFile(join(testDir, 'config.json'), JSON.stringify({ authMode: 'custom' }));
    const config = await loadConfigFromFile(testDir);
  ...
  it('should filter non-string values from trustedProxies', async () => {
    const configData = {
      trustedProxies: ['192.168.1.1', null, 123, '10.0.0.1', undefined, true, '172.16.0.1']
    };

    await writeFile(join(testDir, 'config.json'), JSON.stringify(configData));

    const config = await loadConfigFromFile(testDir);
    expect(config.trustedProxies).toEqual(['192.168.1.1', '10.0.0.1', '172.16.0.1']);
  });
